            ...
            | Root(tries) ->
              if index = 0 then
                if isPredicate(tries, (fun (t : Node) -> (fst t).Character = ch)) then
                  ...
                  let node = Node({ Character = ch; Value = 0 }, [])
                  withNode(Root((inner (index + 1) node immutableBuilder) :: tries))
...
(*
        ...
        for word in words do
          trie <- insert(trie, word)  // mutable way seems to be more clear to read
        trie
*)
